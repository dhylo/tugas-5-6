...
<head>
    ...
    <title>Edit Divisi - Sistem Kepegawaian</title>
...
<body class="bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 min-h-screen">
    ...
    <main class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <!-- Header -->
        <div class="mb-8">
            <h2 class="text-4xl font-bold text-white mb-2">✏️ Edit Divisi</h2>
            <p class="text-gray-400">Perbarui informasi divisi: <span class="font-semibold">{{ $divisi->nama }}</span></p>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-2xl p-8">
            <form action="{{ route('divisi.update', $divisi->id) }}" method="POST">
                ...
                <div class="flex space-x-4">
                    <button 
                        type="submit" 
                        class="flex-1 bg-gradient-to-r from-yellow-500 to-yellow-700 hover:from-yellow-600 hover:to-yellow-800 text-white font-semibold py-3 px-6 rounded-lg shadow-lg transform hover:scale-105 transition duration-200 flex items-center justify-center space-x-2"
                    >
                        <span>✏️</span>
                        <span>Perbarui Divisi</span>
                    </button>
                    <a 
